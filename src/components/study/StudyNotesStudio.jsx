        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 text-cyan-400 font-semibold text-xs tracking-wider uppercase">
              <Sparkles className="w-4 h-4" /> Powered by NVIDIA NIM AI
            </div>
            <h2 className="font-['Space_Grotesk'] text-2xl md:text-3xl font-bold text-white mt-1">
              AI Study Notes & Exam Master
            </h2>
            <p className="text-slate-400 text-sm mt-1 max-w-xl">
              Enter any textbook chapter or topic to generate <strong>Realistic Handwritten Assignment Notes, Visual Concept Flowcharts, Interactive MCQ Quiz, and Expected 2/5-Mark Exam Questions!</strong>
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="px-3.5 py-1.5 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 text-xs font-semibold flex items-center gap-1.5">
              ...
              <span>Assignment & Exam Ready</span>
            </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2 space-y-1.5">
              ...
              <input
                ...
                placeholder="e.g. Structure of Human Heart, French Revolution, Electric Charges & Fields..."
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <option value="NEET / JEE">NEET / JEE / Entrance</option>
                <option value="UPSC / Govt Exam">UPSC / SSC / Govt Exam</option>
                <option value="College / University">College / Engineering</option>
              </select>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-300 flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-500">Leave blank to let AI generate from topic</span>
            </label>
            <textarea
              rows={3}
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder="Paste book text, syllabus points, or summary here..."
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[11px] font-bold text-slate-400">⚡ Quick Try:</span>
              {QUICK_TOPICS.map((q, idx) => (
                <button
                  key={idx}
                  onClick={() => { setTopic(q.title); setGrade(q.grade); handleGenerate(q); }}
                ...
                >
                  <span>{q.icon}</span> {q.title.split(' ')[0]}
                </button>
              ))}
            </div>
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  Generate Handwritten Notes & Quiz
                </>
              )}
            </button>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-2 rounded-2xl bg-slate-900/90 border border-white/10">
            <div className="flex flex-wrap gap-1.5">
              ...
              >
                <Layers className="w-4 h-4" /> 📊 Concept Flowchart
              </button>
              ...
              >
                <Award className="w-4 h-4" /> 📋 2/5-Mark Questions
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 px-2">
              <button
                onClick={handlePrint}
                className="px-4 py-2 rounded-xl text-xs font-semibold bg-emerald-600/20 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-600/30 transition-all flex items-center gap-1.5"
              >
          ...
          {/* ─── TAB 1: REALISTIC HANDWRITTEN RULED NOTEBOOK ─── */}
          {activeTab === 'handwritten' && (
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-900/80 border border-white/10 flex flex-wrap items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-4 text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <option value="'Kalam', cursive">Kalam (Student Pen)</option>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-slate-400">Heading Pen:</span>
                    {[
              ...
              {/* 📖 The Realistic Ruled Paper Notebook Canvas */}
              <div 
                ref={printAreaRef}
                style={{
                  ...
                  lineHeight: '32px',
                  boxShadow: '0 20px 50px rgba(0,0,0,0.5), inset 0 0 100px rgba(0,0,0,0.03)'
                }}
                className="rounded-3xl p-8 sm:p-12 relative overflow-hidden min-h-[700px] border border-slate-300 select-text"
              >
                {/* Punch Hole Rings on Left */}
                <div className="absolute left-4 top-12 bottom-12 flex flex-col justify-between pointer-events-none opacity-40">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="w-4 h-4 rounded-full bg-slate-900/80 shadow-inner border border-slate-400"></div>
                  ))}
                </div>
                ...
                <div className="pl-16 flex items-center justify-between border-b border-rose-300 pb-2 mb-6 text-sm font-semibold text-slate-700">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <span><strong>Page No:</strong> 01</span>
                  </div>
                </div>
                ...
                <div className="pl-16 space-y-8">
                  ...
                  <div className="text-center pb-4">
                    <h1 
                      ...
                      className="text-3xl sm:text-4xl font-bold tracking-wide uppercase inline-block border-b-2 border-rose-400 pb-1"
                    >
                      ★ {result.chapterTitle} ★
                    </h1>
                    {result.keyTakeaway && (
                      <p className="text-lg italic mt-2 opacity-90">
                        "{result.keyTakeaway}"
                      </p>
                    )}
                  </div>
            ...
            <div className="p-6 md:p-8 rounded-3xl bg-slate-900/80 border border-white/10 space-y-6">
              <div className="text-center space-y-1">
                <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 text-xs font-semibold">
                  <Layers className="w-3.5 h-3.5 text-cyan-400" /> Visual Step-by-Step Architecture
                </div>
                <h3 className="text-xl font-bold text-white font-['Space_Grotesk']">
                  {result.diagram?.title || `${result.chapterTitle} — Concept Map`}
                </h3>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-4 border-b border-white/10">
                <div>
                  <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    <HelpCircle className="w-5 h-5 text-amber-400" /> Practice Exam Quiz
                  </h3>
                  <p className="text-xs text-slate-400">Click any option to test your understanding</p>
                </div>
          ...
          {activeTab === 'questions' && (
            <div className="p-6 md:p-8 rounded-3xl bg-slate-900/80 border border-white/10 space-y-6">
              <div className="space-y-1 pb-4 border-b border-white/10">
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <Award className="w-5 h-5 text-emerald-400" /> Most Expected Board / Exam Questions
                </h3>
